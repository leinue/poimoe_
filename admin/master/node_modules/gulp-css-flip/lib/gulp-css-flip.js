...
function gulp(opt) {
    ...
    function modifyContents(file, cb){
        ...
        if(file.isStream()) {
            ...
            file.contents = file.contents.pipe(new BufferStreams(cssFlipTransform(opt)));
        ...
        var flippedCss = flip(String(newFile.contents), opt);

        newFile.contents = new Buffer(flippedCss);
 ...
 * @returns {Function}
 */
function cssFlipTransform(opt) {

    // Return a callback function handling the buffered content
    return function(err, buf, cb) {

        // Handle any error
        if(err) cb(gutil.PluginError('gulp-css-flip', err));

        // Use the buffered content

        var flippedCss = flip(String(buf), opt);

        buf = Buffer(flippedCss);
